...
define(['i18n!orion/navigate/nls/messages', 'orion/webui/littlelib', 'orion/section', 'orion/explorers/explorer', 'orion/explorers/navigatorRenderer', 'orion/explorers/navigationUtils'],
function(messages, lib, mSection, mExplorer, mNavRenderer, mNavUtils){
	...
	NavOutlineRenderer.prototype.getCellElement = function(col_no, item, tableRow){
		...
		if (item.directory) {
			link = mNavRenderer.createLink(("/edit/edit.html"), { Name: item.name, ChildrenLocation: item.path, Directory: true }, this.commandService, this.contentTypeService); //$NON-NLS-0$
		} else if (item.path) {
			...
			href = ("/edit/edit.html") + "#" + item.getProperty("top"); //$NON-NLS-2$ //$NON-NLS-1$ //$NON-NLS-0$
